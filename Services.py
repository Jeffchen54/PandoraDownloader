...
class Config(Enum):
    ...
    # BASIC CONFIGS ##################################################################################
    DOWNLOAD_FOLDER = (('-d', '--download_folder'), 6, Service.BASIC,"<path> : REQUIRED - Set download path for single instance, must use '\\' or '/'")                      #'./' or '../' or '../folder/file.txt' or 'C:/user/folder' 
    VERBOSE = (('-v', '--verbose'), 1, Service.BASIC, ": 0 for no output, 1 for important info, 2 for important info and warnings, 3 for all")                                 # 0 for no output, 1 for important info, 2 for important info and warnings, 3 for all
    THREAD_COUNT = (('-t', '--threads'), 1, Service.BASIC, "<#> : Change download thread count (default is 6)")                                                                # 6 for six threads
    DOWNLOAD_CHUNK_SZ = (('-c', '--chunk_sz'), 1, Service.BASIC, "<#> : Adjust download chunk size in bytes (Default is 64M)")                                                 # 6400000 for ~64MBs
    DOWNLOAD_URL = (('-f', '--bulk'), 5, Service.BASIC, "<textfile.txt> or <url>: Bulk download from text file containing links")                                              # './bulkfile.txt' or 'https://somesite.com'
    UNZIP = (('-u', '--unzip'), 0, Service.BASIC, ": Enables unzipping of files automatically, requires 7z and setup to be done correctly")                                    # True to unzip, false to no unzip
    HTTPS_CODES = (('-z', '--http_codes'), 4, Service.BASIC, "\"500, 502,...\" : HTTP codes to retry downloads on, default is 429 and 403")                                   # [500, 502]
    HTTPS_RETRIES = (('-r', '--http_retries'), 1, Service.BASIC, "<#> : Maximum number of HTTP code retries, default is infinite")                                           # 100 for 100 retries
    HELP = (('-h',), -1, Service.BASIC, ": Help")                                                                                                                                # Display help options and halt program execution
    
    # KEMONO SPECIAL CONFIG ##########################################################################
    KEMONO_FNAME_TYPE = (('--kxftype',), 2, Service.KEMONO, "<download format> : Custom file name, tokens-> [#] counter, [server] -> server name")                               #'[#] - [server]' or '[#]' or [server]
    KEMONO_EXCLUDE_FILE = (('--kxfile',), 3, Service.KEMONO, "\"txt, zip, ..., png\" : Exclude files with listed extensions, NO '.'s")                                           #'['png', 'jpg']'
    KEMONO_EXCLUDE_POST = (('--kxpost',), 3, Service.KEMONO, "\"keyword1, keyword2,...\" : Keyword in excluded posts, not case sensitive")                                       #'['2021', 'hello']'
    KEMONO_EXCLUDE_LINK = (('--kxlink',), 3, Service.KEMONO, "\"keyword1, keyword2,...\" : Keyword in excluded link, not case sensitive. Is for link plaintext, not its target") #'['clip', 'mega']'
    KEMONO_FILE_STRUCTURE = (('--kfstructure',), 1, Service.KEMONO, "<#> : 0 for packed, 1 for partial unpacked, 2 for unpacked")                                                #'0 = packed, 1 = partial, 2 = unpacked'
...
def fix_format_generator(initial):
    """
    ...
    """
    counter = initial
    def formatter(switches:tuple, data:int, service:Service.value)->list:
        """
...
def get_dict_table()->dict:
    ...
    """
    formatter = fix_format_generator(0)
    dict_table = [elem for inner in [formatter(switches, data, service) for (switches, data, service, description) in get_all_config()] for elem in inner]

    return (dict(dict_table))

def get_config_table()->dict:
    """
